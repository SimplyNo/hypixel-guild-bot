    ...
    async interval(bot, server, data) {
        ...
        for (let serverConf of servers) {
            ...
            if (serverConf.config.autoRole.logChannel && !logChannel) {
                await bot.config.autoRole.setLogChannel(server.id, undefined)
            }
            ...
            if (!data) {
                ...
                **/
                ...
                if (!deepEqual(guildConfig, serverConf.config.autoRole.config)) {
                    await bot.config.autoRole.setAutoRoleConfig(serverConf.id, guildConfig);
                }
            }
            ...
            if (true) bot.log(`&6[AutoRole] &a${members.size} members fetched.`)
            ...
            async function updateGuildMembers() {
                ...
                for (let index = 0; index < guildData.members.length; index++) {
                    ...
                    if (verifiedUser) {
                        ...
                        if (rankID && !serverConf.config.autoRole?.config[rankID]) return bot.log(`this rank isn't in the autorole config? ${rankID}`)
                        // get the rank role linked to the guild rank
                        let rankRole = rankID ? serverConf.config.autoRole.config[rankID].role : null;
                        // get the member role linked to the guild
                        let memberRole = serverConf.config.autoRole.memberRole;
                        // get the guest role linked to the guild
                        let guestRole = serverConf.config.autoRole.guestRole;
                        // get the rank position (todo: remove editable rank position)
                        let rankPos = rankID ? serverConf.config.autoRole.config[rankID].pos : null;
                        ...
                        if (memberRole) {
                            ...
                            if (!memberRoleObj) await bot.config.autoRole.setMemberRole(server.id, undefined);
                            memberRoles.addRole(memberRole, `Member role`);
                        }
                        // remove guest role
                        if (guestRole) {
                            let guestRoleObj = await member.guild.roles.fetch(guestRole);
                            if (!guestRoleObj) await bot.config.autoRole.setGuestRole(server.id, undefined);
                        ...
                        if (time && serverConf.config.timeRoles) {
                            let days = Math.floor((Date.now() - time) / 1000 / 60 / 60 / 24);
                            let timeRoles = serverConf.config.timeRoles.sort((a, b) => a.days - b.days);
                        ...
                        // gxp role
                        if (serverConf.config.gxpRoles) {
                            let memberGXP = user.weekly;
                            let gxpRoles = serverConf.config.gxpRoles.sort((a, b) => a.gxp - b.gxp);
                        ...
                        Object.entries(serverConf.config.autoRole.config).forEach((ROLE) => {
            ...
            if (serverConf.config.autoRole.lastUsers) {
                // console.log('----------------- DEBUG MARKER -------------------\nthey have last users\n---------------------------')

                // get people who left guild OR unverified
                let difference = serverConf.config.autoRole.lastUsers.filter((user) => !verifiedUsers.some(currentuser => currentuser.uuid == user.uuid));
                ...
                difference.forEach(async user => {
                    ...
                    let allGuildRoles = Object.entries(serverConf.config.autoRole.config).filter(el => el[1].role);
                    ...
                    let str = `${member.toString()} received the following role changes:\n\n`;
                    // rmove member role
                    if (serverConf.config.autoRole.memberRole) {
                        memberRoles.removeRole(serverConf.config.autoRole.memberRole, `Member role`);
                    }
                    ...
                    // add guest role 
                    if (serverConf.config.autoRole.guestRole) {
                        memberRoles.addRole(serverConf.config.autoRole.guestRole, `Guest Role`);

                    }
                    // remove time role
                    if (serverConf.config.timeRoles) {
                        serverConf.config.timeRoles.forEach(e => {
                    ...
                    // remove gxp role
                    if (serverConf.config.gxpRoles) {
                        serverConf.config.gxpRoles.forEach(e => {
                            memberRoles.removeRole(e.role, `GEXP roles`);

                        })
                    }
                    // remove verify role
                    if (serverConf.config.verification && serverConf.config.verification.role && !isVerified) {
                        memberRoles.removeRole(serverConf.config.verification.role, `Verified Role`);
                    }
                    // add unverified role
                    if (serverConf.config.verification && serverConf.config.verification.unverified && !isVerified) {
                        memberRoles.addRole(serverConf.config.verification.unverified, `Unverified Role`);
                    }
            ...
            // update users in db
            await bot.config.autoRole.setLastUsers(server.id, verifiedUsers);
            // console.log(serverConf.config.joinLogs)
            // join logs
            if (serverConf.config.joinLogs && serverConf.config.joinLogs.channel) {
                let joinLogsChannel = await bot.channels.fetch(serverConf.config.joinLogs.channel).catch(e => null);
                let lastMembers = serverConf.config.joinLogs.lastMembers;
                let lastUpdate = serverConf.config.joinLogs.lastUpdate || 0;
                let lastGuild = serverConf.config.joinLogs.lastGuild;
                ...
                await bot.config.joinLogs.setLastMembers(server.id, guildData.members.map(e => e.uuid), guildData._id);
            }
            // currentServer++;
        }
        if (updateGroup) {
