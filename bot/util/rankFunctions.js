...
const rankFunctions = {
  ...
  getEmojiRank: (player) => {
    ...
    if (rank == "MVP+")
      return (
        ranks["MVP+"].START.join("") +
        ranks["MVP+"][player.plusColor.id || "RED"].join("")
      );
    else if (rank == "MVP++") {
      if (player.monthlyRankColor == "AQUA")
        return (
          ranks["MVP++"].AQUA.START.join("") +
          ranks["MVP++"].AQUA[player.plusColor.id || "RED"].join("")
        );
      return (
        ranks["MVP++"].GOLD.START.join("") +
        ranks["MVP++"].GOLD[player.plusColor.id || "RED"].join("")
      );
    } else {
  ...
  getEmojiRankFromFormatted: (rank = "", name) => {
    ...
    const blankRank = colorCodeRank.replace(
    ...
    if (blankRank == "MVP+")
      return (
        ranks["MVP+"].START.join("") +
        ranks["MVP+"][
          rankFunctions.colorCodeToColor(
            colorCodeRank.substring(6, colorCodeRank.indexOf("+"))
          ) || "RED"
        ].join("")
      );
    else if (blankRank == "MVP++") {
      if (colorCodeRank.substring(0, colorCodeRank.indexOf("[")) == "§b")
        return (
          ranks["MVP++"].AQUA.START.join("") +
          ranks["MVP++"].AQUA[
            rankFunctions.colorCodeToColor(
